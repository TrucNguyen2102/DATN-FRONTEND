...
//                                                 <td className="flex py-2 px-4 border-b border-r text-center justify-center">
//                                                     {booking.status === "Đã Thanh Toán" ? (
//                                                         <button
//                                                             onClick={() => handleViewInvoice(booking.id, booking.tableIds)}
//                                                             className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-700 flex items-center gap-1"
//                                                         >
...
//                             {bookings.length === 0 ? (
//                                 <div className="col-span-full text-center py-4">Không có lịch sử đặt bàn nào.</div>
//                             ) : (
//                                 bookings.map((booking) => (
//                                     <div key={booking.id} className="bg-white border border-gray-300 p-4 rounded-lg shadow-md hover:shadow-lg transition">
//                                         <h3 className="font-semibold text-xl">Đặt Bàn {booking.id}</h3>
...
//                 // Lọc các đơn chưa hoàn thành (không bao gồm các đơn đã hủy và đã hoàn thành)
//                 sortedBookings = sortedBookings.filter((booking) => 
//                     booking.status !== "Đã Hủy" && booking.status !== "Đã Thanh Toán"
//                 );
...
// import CustomerHeader from "../components/Header/CustomerHeader";
// import CustomerSidebar from "../components/Sidebar/CustomerSidebar";
// import { FaEye, FaSyncAlt, FaExclamationTriangle } from "react-icons/fa";
...
//     const [currentPage, setCurrentPage] = useState(1);
//     const [totalPages, setTotalPages] = useState(1);
//     const [showOldBookings, setShowOldBookings] = useState(false); // Trạng thái hiển thị đơn cũ
//     const pageSize = 5;
...
//                     );
//                 } else {
//                     // Lọc các đơn đã thanh toán và đã hủy khi xem đơn cũ
//                     sortedBookings = sortedBookings.filter((booking) =>
//                         booking.status === "Đã Hủy" || booking.status === "Đã Thanh Toán"
//                     );
//                 }
...
//             fetchBookings();
//         }
//     }, [user, currentPage, showOldBookings]);
...
//     const handleToggleOldBookings = () => {
//         setShowOldBookings(prev => !prev);
//     };
...
//                         </button>
//                         <button 
//                             onClick={handleToggleOldBookings} 
//                             className="ml-4 bg-yellow-500 text-white px-4 py-2 rounded hover:bg-yellow-700 flex items-center gap-1"
//                         >
//                             <FaEye className="text-white" /> Xem Các Đơn Cũ
//                         </button>
//                     </div>

//                     {/* Hiển thị các đơn mới nhất với bố cục nổi bật */}
//                     {loading ? (
//                         <p className="text-center">Đang tải dữ liệu...</p>
//                     ) : (
//                         <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
...
//                                         <div
//                                             key={booking.id}
//                                             className={`bg-white border border-gray-300 p-4 rounded-lg shadow-md hover:shadow-lg transition ${isNewest ? "bg-blue-100 border-blue-500" : "bg-white border-gray-300"}`}
//                                         >
//                                             <h3 className="font-semibold text-xl">Đặt Bàn {booking.id}</h3>
//                                             <p><strong>Thời Gian Đặt:</strong> {new Date(booking.bookingTime).toLocaleString()}</p>
//                                             <p><strong>Thời Gian Hết Hạn:</strong> {booking.expiryTime ? new Date(booking.expiryTime).toLocaleString() : "Chưa Có"}</p>
...
//                     <div className="flex justify-center mt-4">
//                         {/* Chỉ hiển thị phân trang nếu đang xem các đơn cũ */}
//                         {showOldBookings && (
//                             <>
//                                 <button
//                                     onClick={() => handlePageChange(currentPage - 1)}
